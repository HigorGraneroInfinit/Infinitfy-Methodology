...
* GHM    HRDK900416     30/08/2013    Gustavo Henrique Michelotto
*
* Descrição: Carga nos Infotipos 0014, 0015 e 0267
*-----------------------------------------------------------------------

*----------------------------------------------------------------------
* Program
*----------------------------------------------------------------------
REPORT zppyc001 LINE-SIZE 180 NO STANDARD PAGE HEADING.
...
*        Tipo para os registros do arquivo
         BEGIN OF y_data              ,
           linha(125) TYPE c          ,   " Linha do arquivo
         END   OF y_data              ,
...
*        Tipo para sumarização de linhas
         BEGIN OF y_sum               ,
           refer      TYPE char18     ,   " Número pessoal + Rubrica + Mês/Ano referência
           betrg      TYPE p0015-betrg,   " Motante
           anzhl      TYPE p0015-anzhl,   " Quantidade
...
*& Internal Tables
*&---------------------------------------------------------------------*
DATA: it_data TYPE STANDARD TABLE OF y_data,   " Tabela com os registros do arquivo
      it_log  TYPE STANDARD TABLE OF y_log ,   " Tabela com o log do processamento
      it_sum  TYPE STANDARD TABLE OF y_sum ,   " Tabela com registros sumarizados
      it_lin  TYPE STANDARD TABLE OF y_lin ,   " Tabela com linhas processadas
      it_0000 TYPE STANDARD TABLE OF p0000 ,   " Tabela para infotipo 0000
      it_0014 TYPE STANDARD TABLE OF p0014 ,   " Tabela para infotipo 0014
      it_0015 TYPE STANDARD TABLE OF p0015 ,   " Tabela para infotipo 0015
      it_2001 TYPE STANDARD TABLE OF p2001 ,   " Tabela para infotipo 2001
      it_0267 TYPE STANDARD TABLE OF p0267 ,   " Tabela para infotipo 0267
...
DATA: w_data TYPE y_data,   " WA para registro do arquivo
      ...
      v_erro,
      v_message   TYPE   string. " Messagem para documentos inválidos na rede

CONSTANTS: c_nome TYPE rlgrap-filename VALUE '/INTERFACES/HR/COOPERATIVA',
           c_sign                      VALUE 'I',
           c_ast   TYPE filename_al11  VALUE '*',
           c_file_f                    VALUE 'F',
           c_file_min                  VALUE 'f',
           c_extensao TYPE c LENGTH 9  VALUE '.txt', "Extensão do arquivo
           c_flag                      VALUE 'X'   , "Flag
           c_pontov                    VALUE ';'   ,
           c_barra                     VALUE '/'   .
...
PARAMETERS: p_serv RADIOBUTTON GROUP g2  DEFAULT 'X' USER-COMMAND xx1, " Servidor
...
PARAMETERS:
  ...
  p_i15_pa TYPE c RADIOBUTTON GROUP rg1,     " Infotipo 0015 (Pão de Açúcar)
  p_i267   TYPE c RADIOBUTTON GROUP rg1.     " Infotipo 0267
SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT 3(27) text-004.
SELECTION-SCREEN POSITION 32.
PARAMETERS:
  p_mot    TYPE p0267-ocrsn.                 " Motivo para cálc. folha pgto.
...
SELECTION-SCREEN END   OF LINE.
PARAMETERS:
  p_exc    TYPE c AS CHECKBOX.               "Exclusão de lançamentos indevidos
SELECTION-SCREEN END   OF BLOCK b2.
SELECTION-SCREEN BEGIN OF BLOCK b3 WITH FRAME TITLE text-003.
PARAMETERS:
  p_ati    TYPE c RADIOBUTTON GROUP rg2,               " Somente ocupação ativo
  p_dem    TYPE c RADIOBUTTON GROUP rg2 DEFAULT 'X'.   " Somente ocupação demitido
...
* Realiza validação dos campos da tela de seleção
  IF NOT p_i267 IS INITIAL.
    IF p_mot IS INITIAL.
      MESSAGE e252(s#) WITH text-e11.  " Necessário informar motivo para cálculo.
    ENDIF.
    IF p_cat IS INITIAL OR
       p_ide IS INITIAL OR
       p_dat IS INITIAL.
      MESSAGE e252(s#) WITH text-e12.  " Necessário informar Folha pgto. off-cycle.
    ENDIF.
  ENDIF.
...
*   Se o registro já foi processado, pula
    READ TABLE it_lin WITH KEY linha = vl_tabix BINARY SEARCH TRANSPORTING NO FIELDS.
    IF sy-subrc IS INITIAL.
      CONTINUE.
    ENDIF.
...
*   Verifica se o registro já foi processado
    ...
    READ TABLE it_sum ASSIGNING <fs_sum> WITH KEY refer = vl_refer BINARY SEARCH.
    IF sy-subrc IS INITIAL.
*     Verifica se já foi processada esta referencia
      IF NOT <fs_sum>-proce IS INITIAL.
        ...
        CONCATENATE text-s03 <fs_sum>-proce INTO vl_msg SEPARATED BY space.
        PERFORM : zf_atribui_log USING vl_pernr vl_lgart vl_begda vl_endda vl_betrg vl_anzhl vl_afast 'S' vl_msg vl_tabix.
        CONTINUE.
      ELSE.
...
*     Para o infotipo 0267, verifica o motivo e identificador
      ...
      ENDIF.
      IF sy-subrc IS INITIAL.
        PERFORM: zf_atribui_log USING vl_pernr vl_lgart vl_begda vl_endda vl_betrg vl_anzhl vl_afast 'E' text-e07 vl_tabix.
        CONTINUE.
      ELSE.
        CLEAR: <fs_w_infty>.
        IF NOT p_i267 IS INITIAL.
    ...
    ELSE.
      IF NOT <fs_it_infty>[] IS INITIAL.
        DELETE <fs_it_infty> WHERE ('PERNR NE VL_PERNR OR SUBTY NE VL_LGART OR BETRG NE VL_BETRG').
      ENDIF.
      IF <fs_it_infty>[] IS INITIAL.
        PERFORM: zf_atribui_log USING vl_pernr vl_lgart vl_begda vl_endda vl_betrg vl_anzhl vl_afast 'E' text-e08 vl_tabix.
        CONTINUE.
      ENDIF.
    ENDIF.
...
*   Seleciona dados do funcionário
    ...
    IF sy-subrc IS INITIAL.
*     Verifica se o funcionário esta desligado e se a opção escolhida é só de Ativos
      IF w_0000-stat2 EQ 0.
        IF NOT p_i15_pa IS INITIAL OR
           NOT p_ati    IS INITIAL.
          PERFORM: zf_atribui_log USING vl_pernr vl_lgart vl_begda vl_endda vl_betrg vl_anzhl vl_afast 'E' text-e05 vl_tabix.
          CONTINUE.
        ENDIF.
      ENDIF.
    ELSE.
      PERFORM: zf_atribui_log USING vl_pernr vl_lgart vl_begda vl_endda vl_betrg vl_anzhl vl_afast 'E' text-e06 vl_tabix.
      CONTINUE.
    ENDIF.
...
*   Grava registro
    PERFORM zf_grava_registro USING vl_infty vl_pernr vl_betrg vl_lgart vl_begda vl_endda vl_anzhl vl_afast <fs_w_infty> vl_tabix <fs_it_infty>.
...
FORM zf_grava_registro USING p_infty TYPE prelp-infty
  ...
  IF p_exc IS INITIAL.
    ...
    PERFORM: zf_salva_registro USING p_infty p_pernr p_betrg p_lgart p_begda p_endda p_anzhl vl_seqnr p_afast p_recor p_linha.
...
*   Inclui Registro Infotipo
    ...
    IF NOT vl_return IS INITIAL.
      ...
      PERFORM: zf_atribui_log USING p_pernr p_lgart p_begda p_endda p_betrg p_anzhl p_afast 'E' vl_msg p_linha.
      CALL FUNCTION 'BAPI_TRANSACTION_ROLLBACK'.
    ELSE.
*      CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'
*        EXPORTING
*          wait = 'X'.
      PERFORM: zf_atribui_log USING p_pernr p_lgart p_begda p_endda p_betrg p_anzhl p_afast 'S' vl_msg p_linha.
    ENDIF.
...
* Titulo para o report
  ...
  ELSEIF NOT p_i15_pa IS INITIAL.
...
* Imprime o total de erros apresentados no processamento
  WRITE: /01  sy-vline,
          05  text-r08,     " Total de erros apresentados no processamento:
          60  vl_err  ,
          180 sy-vline.
...
*   Imprime Funcionários processados:
    WRITE: /01  sy-vline      ,
            05  '@08@' AS ICON,
            10  text-r09      ,     " Funcionários processados com sucesso:
            180 sy-vline      .
...
*     Trata o layout conforme o tipo escolhido na tela de seleção
    IF NOT p_i15_pa IS INITIAL.
      ch_pernr = p_data-linha+5(8).
    ELSE.
  ...
  IF NOT sy-subrc IS INITIAL.
    ...
    PERFORM: zf_atribui_log USING ch_pernr ch_lgart ch_begda ch_endda ch_betrg ch_anzhl space 'E' vl_msg p_tabix.
    ch_conv = 'E'.
    RETURN.
  ENDIF.
...
* Atribui montante convertendo
  CATCH SYSTEM-EXCEPTIONS convt_no_number = 1
    ...
    IF     NOT p_i14 IS INITIAL.
      ...
      ch_betrg = vl_aux.
    ELSEIF NOT p_i15_pa IS INITIAL.
      CONCATENATE p_data-linha+105(9) '.' p_data-linha+113(2) INTO vl_aux.
      ch_betrg = vl_aux.
    ELSE.
...
* Atribui datas convertendo
  CATCH SYSTEM-EXCEPTIONS convt_no_number = 1
    ...
    IF NOT p_i14  IS INITIAL OR
      ...
      CONCATENATE p_data-linha+16(4) p_data-linha+14(2) p_data-linha+12(2) INTO ch_begda.
      IF NOT p_i14 IS INITIAL.
*       Regra: Se o Infotipo for IT0014 e a rubrica 4036, o ENDDA será formado pelo BEGDA + ANZHL
        IF ch_lgart = '4036'.
          vl_mes = ch_anzhl.
          CALL FUNCTION 'ISH_DATE_ADD_MONTH'
            ...
            EXCEPTIONS
              ...
              OTHERS     = 2.
        ELSE.
          CONCATENATE p_data-linha+24(4) p_data-linha+22(2) p_data-linha+20(2) INTO ch_endda.
        ENDIF.
      ELSE.
        ch_endda = '99991231'.
      ENDIF.
...
***     Validar extensão e nomenclatura do arquivo.
        ...
        ELSE.
          CONCATENATE v_message w_file-name c_pontov INTO v_message SEPARATED BY space.
        ENDIF.
